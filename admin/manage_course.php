...
<div class="teacher_info">
	...
	<table>
        ...
        <?php
			...
			{
				$sql = "select count(*) amount from teacher where teacher_name like '%".$_GET['teacher_name']."%'";
				//echo $sql;
			}
			else
			{
			...
			if( $amount )
			{
				//where teacher_id not like '0000_'
   				if(isset($_GET['teacher_name']))
				{
					$sql = "select * from teacher  where teacher_id not like '0000_' and teacher_name like '%".$_GET['teacher_name']."%' order by teacher_id asc limit ". ($page-1)*$page_size .", ".$page_size;
				}
				else
				{
